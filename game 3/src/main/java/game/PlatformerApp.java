...
//        getGameWorld().addEntity(trafficLight);
...
//                System.out.println(count);
        ...
        FXGL.getPhysicsWorld().addCollisionHandler(new CollisionHandler(EntityType.PLAYER, EntityType.TRAFFIC_LIGHT) {
            @Override
            protected void onCollision(Entity player, Entity trafficLight) {
                boolean isGreen = trafficLight.getComponent(TrafficLightComponent.class).isGreen();
                if (!isGreen){
                    onPlayerDied();
                    spawnKey();
                }
        ...
        FXGL.getPhysicsWorld().addCollisionHandler(new CollisionHandler(EntityType.PLAYER, EntityType.LAVA) {
            @Override
            protected void onCollision(Entity player, Entity lava) {
                onPlayerDied();
            }
        });
    }
    ...
    private void spawnLight(){
        switch(map_level){
            case 1:
                key = spawn("trafficlight", 650,0);
                break;
            case 2:
                key = spawn("trafficlight", 450,0);

                break;
            case 3:
                key = spawn("trafficlight", 370,340);
                break;
            case 4:
                key = spawn("trafficlight", 485,440);
                break;
        }
    }
